...
public:
	...
	{
		T myValues[4];
		// Store quaternions as WXYZ to match the rest of the math code.
...
inline Vector3<T> Quaternion<T>::GetEulerAnglesRadians() const
{
	...
	if (abs(sinp) >= T(1))
	{
		yaw = copysign(Math::Pi * T(0.5), sinp); // use 90 degrees if out of range
	}
	else
	{
		yaw = asin(sinp);
